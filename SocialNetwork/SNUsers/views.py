...
def FollowUser(request,loggedin_user,user):
    ...
    '''
    try:
        cur_user = SNUser.objects.get(username=loggedin_user)
        fol_user = SNUser.objects.get(username=user)
        cur_user.following.add(fol_user)
        cur_user.save()
        serializer = SNUserSerializer(cur_user)
...
def GetFollowers(request,username):
    ...
    try:
        ...
        serializer = SNUserSerializer(followers, many=True)
...
def GetFollowing(request,username):
    ...
    try:
        ...
        serializer = SNUserSerializer(following, many=True)
        return Response(serializer.data)
    except Exception as e:
        error = {'Error_code': status.HTTP_400_BAD_REQUEST,
                        'Error_Message': "User does not exist"}
...
def users(request):
    ...
    if request.method == 'GET':
        users = SNUser.objects.all()
        serializer = SNUserSerializer(users, many=True)
        return Response(serializer.data)        